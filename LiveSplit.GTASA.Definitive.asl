...
startup
{
	...
	vars.truckingOffset = 9581; // default offset, may be changed in init based on version
	...
	vars.missions = new Dictionary<int, Dictionary<int, string>> {
		...
		{vars.truckingOffset, new Dictionary<int, string> { // $TRUCKING_TOTAL_PASSED_MISSIONS (version 1.0.113.21181)
			{1, "Trucking 1"},
			{2, "Trucking 2"},
			{3, "Trucking 3"},
			{4, "Trucking 4"},
...
*/
	...
	addMissionsHeader("Trucking", vars.truckingOffset, "Trucking");
...
init
{
	...
	// Version-specific addresses and offsets
	//========================================
	int statBaseAddr;
	int scriptBaseAddr;
	int startAddr;
	...
	if (version == "1.0.113.21181")
	{
		...
		startOffset = getValueFromPattern(21, "8b 05 ?? ?? ?? ?? ?? 8b ?? ?? ?? ?? ?? 83 f8 08 73 ?? ?? c6 ?? ?? ?? ?? ?? 01");
		threadAddr = getAddressFromPattern(9, "?? 53 ?? 83 ec ?? ?? 8b 15 ?? ?? ?? ?? ?? 85 d2 74 ?? 33 c9 ?? 8d 05 ?? ?? ?? ??");
		loadingAddr = getAddressFromPattern(3, "0f b6 ?? ?? ?? ?? ?? ?? 88 ?? ?? ?? ?? ?? ?? 89 ?? ?? ?? ?? ?? 66 89 ?? ?? ?? ?? ?? ?? 88");
		playTimeAddr = getAddressFromPattern(2, "8b 15 ?? ?? ?? ?? ?? 85 c9 74 ?? 8b 05 ?? ?? ?? ?? 05 20 bf 02 00");
		tagsAddr = (int)0x572B41C;
		truckingOffset = 9587;
	}
	else
	{
